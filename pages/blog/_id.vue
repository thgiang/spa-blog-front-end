<template>
  <div class="blog">
    <div class="blog-content">
      <div
        v-if="this.$store.state.auth.user.role === 'admin' || (this.$store.state.auth.user.role === 'writer' && this.$store.state.blog.blog.user_id === this.$store.state.auth.user.id)"
        class="float-right">
        <nuxt-link :to="{name: 'blog-manager-edit-id', params: {id: this.$store.state.blog.blog.id}}"
                   class="bg-danger p-1 text-white">Sửa bài viết
        </nuxt-link>
      </div>
